...
if($dopost=="addArchives")
{
    ...
    //跳转并传递参数
 // echo "{$gurl}?channelid={$channelid}&cid={$cid}";
 //  die();
...
else if($dopost=="listGas")
{
    ...
    if(empty($gurl)) $gurl = 'content_gas_list.php';
    header("location:{$gurl}?channelid={$channelid}&cid={$cid}");
    exit();
}
...
else if($dopost=="listzpinfo")
{
    ...
    if(empty($gurl)) $gurl = 'content_zpinfo_list.php';
    header("location:{$gurl}?channelid={$channelid}&cid={$cid}");
    exit();
}
