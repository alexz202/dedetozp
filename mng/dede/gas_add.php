...
if($dopost!='save')
{
    ...
    include DedeInclude("templets/gas_add.htm");
    exit();
}
...
else if($dopost=='save')
{
    ...
    if(isset($_FILES)){
        //uploadfile
        $file=$_FILES['file'];
        $path=$cfg_basedir."/dedetozp/mng/uploads/files/";
        $res= uploadFIle($file,$path);
        if($res){
            $filename=$file['name'];
        }
    }
    ...
    //保存到主表
    $query = "INSERT INTO `#@__gas`(id,typeid,typeid2,sortrank,flag,ismake,channel,arcrank,click,money,title,shorttitle,
    ...
    //返回成功信息
    $msg = "    　　请选择你的后续操作：
    <a href='gas_add.php?cid=$typeid'><u>继续发布文章</u></a>
    &nbsp;&nbsp;
    <a href='$artUrl' target='_blank'><u>查看文章</u></a>
    &nbsp;&nbsp;
    <a href='gas_do.php?aid=".$arcID."&dopost=editArchives'><u>更改文章</u></a>
    &nbsp;&nbsp;
    <a href='catalog_do.php?cid=$typeid&dopost=listGas'><u>已发布内容管理</u></a>
    &nbsp;&nbsp;
    $backurl
  ";
