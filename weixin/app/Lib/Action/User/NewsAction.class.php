<?php
class NewsAction extends UserAction{
...
//		}
	...
	public function index(){
		...
		$news=M('news');
        $dbname= $news->getTableName();
		if(!empty($_REQUEST['search'])){
			$map=array();
			$sqlcount="select count(id) as count from $dbname where 1=1 and title='".trim($_REQUEST['search'])."' or city= '".trim($_REQUEST['search'])."'";
			import('ORG.Util.Page');
			$count=$news->query($sqlcount);
			$count=$count[0]['count'];
			...
			$show=$page->show();
			$sql="select * from $dbname where 1=1 and title='".trim($_REQUEST['search'])."' or city= '".trim($_REQUEST['search'])."' order by updatetime desc,id desc limit $page->firstRow,$page->listRows";
			$list=$news->query($sql);
	...
	public function insert(){
		...
		//	$data['content']=$content;
			...
			if($result){
				$this->success(L('update_success'),U('News/index', array(
                    'token' => $this->token,
                )));
			}
			else
	...
	public function update(){
		...
		if(!empty($_GET['id'])){
			...
			$results=$news->where($condition)->select();
		...
		if(!empty($_POST['id'])){
			...
			if($result!=false){
				...
				//  echo 'false';
                $this->success(L('update_fail'),U('News/index', array(
                    'token' => $this->token,
                )));
			}
...
//	}
	...
	public function getOnenewsSP(){
		if(!empty($_POST['newsid'])){
		$sp=M('spinfo');
		$condition['newsid']=$_POST['newsid'];
	...
	public function addspajax(){
	  ...
	  $result= $sp->add($date);
		if($result){
				//$this->assign("jumpUrl",$return_url);
				$this->assign("jumpUrl",'__ROOT__/index.php?m=news&a=spinfo&id='.$newsid);
	...
	if($result){
				//$this->assign("jumpUrl",$return_url);
				$this->assign("jumpUrl",'__ROOT__/index.php?m=news&a=spinfo&id='.$spid);
				$this->success(L('update_success'));
			}else{
				//  echo 'false';
				$this->assign("jumpUrl",'__ROOT__/index.php?m=news&a=spinfo&id='.$spid);
