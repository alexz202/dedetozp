...
class MapAction extends Action{
    ...
    public function nearest_jx($x,$y){
        //
        $company_model=M('Company');
       // $where=array('token'=>$this->token);
        //todo limit area
        $neararr=$this->getAround($x,$y,20000);
       // return array($neararr[0].'|'.$neararr[1],'text');
        $dbname= $company_model->getTableName();
        $sql="select * from ".$dbname." where latitude between $neararr[0] and  $neararr[1] and longitude between $neararr[2] and  $neararr[3] order by  isbranch ASC,taxis ASC";
        $companies= $company_model->query($sql);
        //$companies=$company_model->order('isbranch ASC,taxis ASC')->select();
        $ldistance=0;
        $nearestCompany=array();
...
//            var_dump($nearestDistance);
//            var_dump($nearestCompany);
            $return=array();
          if(is_array($nearestDistance)){
              sort($nearestDistance);
              $arr=array();
              foreach($nearestDistance as $v){
                  if($index<3){
                      $distance=$this->_getDistance($v);
                      $id=$nearestCompany[$v]['id'];
                      $name=$nearestCompany[$v]['name'];
                      $describe='';
                      $imgurl='';
                      $url=C('MAPPURL').'weixin/index.php?g=Home&m=WXUser&a=getcompanyone&id='.$id;
                      if($index==0)
                      $imgurl=C('MAPPURL').'weixin/images/123.jpg';
                      $return[]=array($name.'店，距离您'.$distance,$describe,$imgurl,$url);
                      $index++;
                      continue;
                  }
              }
